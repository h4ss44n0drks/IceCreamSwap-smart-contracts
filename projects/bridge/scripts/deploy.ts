...
async function main() {
  ...
  const ERC20Handler = await ethers.getContractFactory("ERC20NativeHandler");
  const erc20Handler = await ERC20Handler.deploy(bridge.target, bridgeConfig.tokenFeePercent * 100);
  console.log(`ERC20NativeHandler deployed to ${erc20Handler.target}`);
  ...
  for (const tokenConfig of bridgeConfig.bridgedTokens) {
    ...
    console.log(`deployed and configured ${tokenConfig.symbol} token at ${bridgedToken.target}`);
    tokens[tokenConfig.symbol] = bridgedToken.target;
  }
  ...
  const contracts = {
    ...
    erc20Handler: erc20Handler.target,
    tokens: tokens,
  };
